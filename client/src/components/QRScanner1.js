...
export default function QRScanner({ onScan, onClose }) {
  ...
  useEffect(() => {
    ...
    // Since permission is already granted from parent, start scanning immediately
  ...
  const startScanning = useCallback(async () => {
    ...
    try {
      ...
      if (isMobile) {
        ...
        // Only add deviceId if we have one and it's not empty
  ...
  const startScanningBasic = useCallback(async () => {
    try {
      ...
      // Very basic constraints as fallback - should work on most devices
      ...
      setError('Unable to access camera even with basic settings. Please check permissions and try again.')
      setIsScanning(false)
    }
  }, [scanFrame])
  ...
  const scanFrame = useCallback(() => {
    ...
    // Scan for QR code with more aggressive settings for mobile
